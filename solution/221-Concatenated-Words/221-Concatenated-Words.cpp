// 221-Concatenated-Words.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
public:
    ...
    vector<string> findAllConcatenatedWordsInADict(vector<string>& words) {
        ...
        unordered_set<string> strset(words.begin(), words.end());
        ...
        {
            dfs(words[i], strset, result, 0, 0, path, res);
        }
    ...
    void dfs(string& word, unordered_set<string>& strset, vector<string>& result,
        int pos, int depth, vector<string> &path, unordered_set<string> &res)
    {

        if (res.count(word) > 0)
            return;

        if (pos == word.size() && depth >= 2)
        {
            ...
            for (int i = 1; i + pos <= word.size(); i++)
            {
                string substr = word.substr(pos, i);

                if (substr == word)
                    continue;

                if (strset.count(substr) > 0)
                {
                    path.push_back(substr);
                    dfs(word, strset, result, pos + i, depth + 1, path, res);
                    path.pop_back();
                }
